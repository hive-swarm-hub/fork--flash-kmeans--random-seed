#!/usr/bin/env python3
"""Test: Skip x_sq in inner loop distance computation (since argmin is over K, not N)."""
import torch
import triton
...
def _euclid_assign_v2(
    ...
    """Optimized: don't add x_sq in inner loop (it's constant across K)."""
    ...
    # Don't need x_sq in inner loop - it's constant across K
    # Just track best (c_sq - 2*cross) which gives same argmin
    ...
    for k_start in range(0, K, BLOCK_K):
        ...
        # neg_score = c_sq - 2*cross (lower is better, same as original argmin)
...
def verify(wl):
    ...
    x_sq = compute_sq_norms(x)
    c_sq = compute_sq_norms(centroids)
    out1 = torch.empty((B, N), device="cuda", dtype=torch.int32)
    out2 = torch.empty((B, N), device="cuda", dtype=torch.int32)
    config = {"BLOCK_N": 128, "BLOCK_K": 128, "num_warps": 4, "num_stages": 1}
    grid = lambda META: (triton.cdiv(N, META["BLOCK_N"]), B)

    _euclid_assign_kernel[grid](x, centroids, x_sq, c_sq, out1, B, N, K, D,
        x.stride(0), x.stride(1), x.stride(2),
        centroids.stride(0), centroids.stride(1), centroids.stride(2),
        x_sq.stride(0), x_sq.stride(1), c_sq.stride(0), c_sq.stride(1),
        out1.stride(0), out1.stride(1), **config)

    _euclid_assign_v2[grid](x, centroids, x_sq, c_sq, out2, B, N, K, D,
        x.stride(0), x.stride(1), x.stride(2),
        centroids.stride(0), centroids.stride(1), centroids.stride(2),
        x_sq.stride(0), x_sq.stride(1), c_sq.stride(0), c_sq.stride(1),
        out2.stride(0), out2.stride(1), **config)
...
for wl in WORKLOADS:
    ...
    ms_v2 = bench(_euclid_assign_v2, wl, config)
    print(f"  Original: {ms_orig:.3f} ms")
    print(f"  V2 (no x_sq): {ms_v2:.3f} ms  ({(ms_v2/ms_orig - 1)*100:+.1f}%)")
