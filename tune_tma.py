#!/usr/bin/env python3
"""Test: TMA block pointers for centroid loading."""
import torch
import triton
...
def _euclid_assign_tma(
    ...
    # Block pointer for centroid tiles - enables TMA on H100
    c_block_ptr = tl.make_block_ptr(
        base=c_ptr + pid_b * stride_c_b,
        shape=(D, K),
        strides=(stride_c_d, stride_c_k),
        offsets=(0, 0),
    ...
    for k_start in range(0, K, BLOCK_K):
        ...
        # TMA load of centroid tile (D, BLOCK_K)
        c_tile = tl.load(c_block_ptr, boundary_check=(1,), padding_option='zero', eviction_policy='evict_last')
...
def bench(kernel, wl, num_warmup=5, num_timed=20):
    ...
    try:
        for _ in range(num_warmup):
            kernel[grid](
                x, centroids, x_sq, c_sq, out, B, N, K, D,
...
for wl in WORKLOADS:
    print(f"\n=== {wl['label']} ===")
    ms_orig = bench(_euclid_assign_kernel, wl)
    ms_tma = bench(_euclid_assign_tma, wl)
    if ms_orig and ms_tma:
        print(f"  Original: {ms_orig:.3f} ms")
        print(f"  TMA:      {ms_tma:.3f} ms  ({(ms_tma/ms_orig - 1)*100:+.1f}%)")
