#!/usr/bin/env python3
"""Test warp specialization with various num_warps."""
import torch
import triton
...
def _euclid_assign_ws(
    ...
    for k_start in tl.range(0, K, BLOCK_K, num_stages=NUM_STAGES, warp_specialize=True):
...
# Also test without warp_specialize but with tl.range num_stages
@triton.jit
def _euclid_assign_pipeline(
    x_ptr, c_ptr, x_sq_ptr, c_sq_ptr, out_ptr,
    B: tl.constexpr, N: tl.constexpr, K: tl.constexpr, D: tl.constexpr,
    stride_x_b: tl.constexpr, stride_x_n: tl.constexpr, stride_x_d: tl.constexpr,
    stride_c_b: tl.constexpr, stride_c_k: tl.constexpr, stride_c_d: tl.constexpr,
...
from flash_kmeans.assign_euclid_triton import _euclid_assign_kernel

WL = {"label": "stress", "B": 4, "N": 262144, "D": 128, "K": 8192}

print(f"=== {WL['label']} ===")

# Baseline
ms = bench(_euclid_assign_kernel, WL, {"BLOCK_N": 128, "BLOCK_K": 128, "num_warps": 4, "num_stages": 1})
print(f"  Baseline (range, K constexpr): {ms}")

# Warp specialize with different warps
for nw in [8, 12, 16]:
    for ns in [2, 3]:
        for bk in [64, 128]:
            cfg = {"BLOCK_N": 128, "BLOCK_K": bk, "NUM_STAGES": ns, "num_warps": nw, "num_stages": 1}
            ms = bench(_euclid_assign_ws, WL, cfg)
            print(f"  WS warps={nw} stages={ns} BK={bk}: {ms}")

# Pipeline only (no warp spec)
for nw in [4, 8]:
    for ns in [2, 3]:
        cfg = {"BLOCK_N": 128, "BLOCK_K": 128, "NUM_STAGES": ns, "num_warps": nw, "num_stages": 1}
        ms = bench(_euclid_assign_pipeline, WL, cfg)
        print(f"  Pipeline warps={nw} stages={ns}: {ms}")

torch.cuda.empty_cache()

# Also test large-dense
WL2 = {"label": "large-dense", "B": 32, "N": 65536, "D": 128, "K": 4096}
print(f"\n=== {WL2['label']} ===")
ms = bench(_euclid_assign_kernel, WL2, {"BLOCK_N": 128, "BLOCK_K": 128, "num_warps": 4, "num_stages": 1})
print(f"  Baseline: {ms}")

for nw in [8, 16]:
    for ns in [2, 3]:
        cfg = {"BLOCK_N": 128, "BLOCK_K": 128, "NUM_STAGES": ns, "num_warps": nw, "num_stages": 1}
        ms = bench(_euclid_assign_ws, WL2, cfg)
        print(f"  WS warps={nw} stages={ns}: {ms}")
